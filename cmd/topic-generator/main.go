...
func main() {
	...
	if err := godotenv.Load(); err != nil {
		slog.Warn("No .env file found, relying on environment variables from system")
	}
	...
	config.LoadEnv(appEnv) // config.LoadEnv might also load .env, ensure order is fine or it's idempotent
	logging.InitLogger()
